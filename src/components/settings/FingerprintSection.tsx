...
export function FingerprintSection() {
  ...
  return (
    <section className="app-panel rounded-2xl border border-border/60 bg-card/92 p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2.5 pb-3 border-b border-border/40"><ShieldCheck className="size-5 text-emerald-500" /><div><h3 className="text-sm font-bold">Fingerprint Login</h3><p className="text-[11px] text-muted-foreground">Unlock with this device's fingerprint or face unlock.</p></div></div>
      {credentials.map(credential => <div key={credential.id} className="flex items-center justify-between bg-muted/20 border px-3 py-2.5 rounded-xl text-xs"><span className="flex items-center gap-2 font-semibold"><Fingerprint className="size-4 text-emerald-500" />{credential.deviceLabel || 'Registered device'}</span><button type="button" onClick={() => void remove(credential.id)} disabled={hideSensitive} className="p-1.5 text-muted-foreground hover:text-orange-500 disabled:opacity-40"><Trash2 className="size-3.5" /></button></div>)}
      {enrolledHere
        ? <div className="flex justify-center gap-1.5 text-[11px] font-semibold text-emerald-500"><CheckCircle2 className="size-3.5" /> Enabled on this device</div>
        : <button type="button" onClick={() => void enroll()} disabled={busy || hideSensitive} className="w-full inline-flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-bold bg-emerald-600 disabled:opacity-50 text-white">{busy ? <span className="size-3.5 rounded-full border-2 border-t-transparent animate-spin" /> : <Fingerprint className="size-3.5" />}{credentials.length ? 'Add another device' : 'Enable on this device'}</button>}
    </section>
  )
}
