    ...
    <section aria-labelledby="category-review-heading" className={`relative space-y-3 rounded-xl border border-primary/25 bg-primary/5 p-3.5 ${reviewing ? 'perimeter-beam-host' : ''}`}>
      ...
      <header className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[10px] text-muted-foreground">Review category purpose, flow, and recent usage before applying anything.</p>
        </div>
      ...
      {reviewing && <div className="flex items-center gap-2 rounded-lg border border-border/50 bg-background/50 p-3 text-[11px] font-semibold text-muted-foreground"><Loader2 className="size-3.5 animate-spin text-accent-ink" />Reviewing category names, flows, and usage…</div>}
      {error && <p role="alert" className="flex items-center gap-1 rounded-lg border border-orange-500/25 bg-orange-500/8 p-3 text-[11px] font-semibold text-orange-600 dark:text-orange-400"><AlertCircle className="size-3.5 shrink-0" />{error}</p>}
      {!reviewing && !error && suggestions.length === 0 && <p className="flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/8 p-3 text-[11px] text-muted-foreground"><CheckCircle2 className="size-3.5 shrink-0 text-emerald-500" />No cleanup or flow corrections to propose.</p>}
          ...
          <article key={suggestion.id} className="overflow-hidden rounded-xl border border-border/60 bg-background/75">
            <div className="space-y-2.5 p-3">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="rounded-full border border-blue-500/25 bg-blue-500/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-blue-600 dark:text-blue-400">
                  {suggestion.type === 'changeFlow' ? 'Flow correction' : suggestion.type}
                </span>
                <span className="text-[9px] font-semibold text-muted-foreground">{confidence}% confidence</span>
              </div>
              <div>
                <h5 className="text-xs font-bold text-foreground">{suggestion.title}</h5>
                <p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">{suggestion.summary}</p>
              </div>
              <div className="flex flex-wrap items-center gap-1.5">
                {suggestion.categories.map(name => <Button variant="unstyled" key={name} type="button" onClick={() => onNavigateToLedger?.({ category: name, showAllCycles: true })} className={`press-scale inline-flex min-h-8 items-center rounded-md border px-2 text-[10px] font-semibold ${getCategoryBadgeClass(name)}`}>{name}</Button>)}
                {suggestion.type === 'changeFlow' && <span className="inline-flex min-h-8 items-center gap-1 rounded-md border border-border/60 bg-muted/30 px-2 text-[10px] font-semibold text-muted-foreground"><ArrowLeftRight className="size-3" />{flowLabel(suggestion.sourceFlow)} → <strong className="text-foreground">{flowLabel(suggestion.targetFlow)}</strong></span>}
              </div>
              {suggestion.type === 'consolidate' && <label className="block space-y-1 text-[10px] font-semibold text-muted-foreground">Move its entries to:<CustomSelect ariaLabel="Category consolidation target" value={target} onChange={value => setConsolidateTargets(previous => ({ ...previous, [suggestion.id]: String(value) }))} options={[{ value: '', label: 'Choose a category' }, ...consolidateOptions.map(category => ({ value: category.name, label: category.name }))]} className="w-full" /></label>}
            </div>
            <footer className="flex items-center justify-between gap-2 border-t border-border/50 bg-muted/15 px-3 py-2.5">
              <Button variant="unstyled" type="button" disabled={suggestion.affectedTransactionCount === 0 || !onNavigateToLedger} onClick={() => onNavigateToLedger?.({ category: suggestion.categories[0], txType: incompatibleType, showAllCycles: true })} className="min-h-11 min-w-0 rounded-lg px-2 text-left text-[9px] font-bold uppercase text-orange-600 disabled:text-muted-foreground sm:min-h-8">
