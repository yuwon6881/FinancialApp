...
export const BillTimeline: React.FC<BillTimelineProps> = ({
  ...
  const processedPayments = React.useMemo(() => {
    ...
    // Historical synthesis fallback: If viewing a past cycle where a subscription was deleted,
    // but the Ledger contains a logged transaction for it in this cycle range, synthesize a Paid entry
    ...
    mappedList.forEach(p => {
      const matchingTx = (transactions || []).find(t => {
        ...
        const descLower = (t.description || '').toLowerCase().trim()
        const pNameLower = p.name.toLowerCase().trim()
        return descLower === pNameLower || (pNameLower.length > 2 && descLower.includes(pNameLower))
      })
    ...
    ;(transactions || []).forEach(t => {
      ...
      // Include transactions that were generated by recurring payments or are categorized under recurring/subscriptions
      const isRecurringTx = Boolean((t as any).recurringPaymentId) || 
        (t.ledgerCategory && t.ledgerCategory !== 'Manual') || 
        (t.category && (t.category.toLowerCase().includes('sub') || t.category.toLowerCase().includes('recurring')))
      ...
      if (isRecurringTx) {
        ...
        historicalExtraList.push({
          id: `hist-tx-${t.id}`,
          recurringPaymentId: (t as any).recurringPaymentId || String(t.id),
