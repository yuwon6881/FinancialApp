...
export function AmountReview({ document, updateDocument, currency, disabled = false }: { document: VaultDocument; updateDocument: UpdateDocumentFn; currency?: string; disabled?: boolean }) {
  ...
  if (!editing) return <Button variant="unstyled" type="button" onClick={() => setEditing(true)} disabled={disabled} aria-label={needsReview ? `Review the suggested amount for ${document.originalFileName}` : undefined} className={`inline-flex min-h-11 items-center gap-1.5 rounded-lg px-1 text-xs font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 sm:min-h-8 ${needsReview ? 'border border-amber-500/40 bg-amber-500/10 px-2 text-amber-600 hover:bg-amber-500/20' : 'text-accent-ink hover:bg-accent/60'}`}>
