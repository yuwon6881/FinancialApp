  ...
  return <div className="space-y-4">
    <>
      ...
      {selected ? (
        ...
        {results.map(result => <Button type="button" variant="tertiary" key={`${result.symbol}-${result.mic ?? result.exchange}`} onClick={() => setSelected(result)} className="block w-full cursor-pointer rounded-xl border border-border/50 p-3 justify-start text-left transition-colors hover:bg-muted/30">
          ...
          {result.availabilityMessage && <span className="mt-1 block text-xs text-muted-foreground">{result.availabilityMessage}</span>}
        </Button>)}
      </div>}
