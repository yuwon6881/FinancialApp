...
export function AppProvider({ value, children }: { value: AppContextValue; children: ReactNode }) {
  const prefs = useMemo(() => ({
    ...
    maskPassiveFinancialFigures: value.maskPassiveFinancialFigures ?? value.hideSensitive,
    sensitivePreferenceStatus: value.sensitivePreferenceStatus,
    currency: value.currency,
    darkMode: value.darkMode,
    formatSensitive: value.formatSensitive,
  }), [value.hideSensitive, value.maskPassiveFinancialFigures, value.sensitivePreferenceStatus, value.currency, value.darkMode, value.formatSensitive])
